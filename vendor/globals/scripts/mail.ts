...
/**
 * Rhost Vision: mail.ts
 *
 * Handles @mail command and its subcommands.
 * Override adds staff name color support.
 */
...
// Staff name color helper
function coloredName(obj: IDBObj | undefined | null, fallback: string): string {
    if (!obj) return fallback;
    const rawName = obj.name || fallback;
    const nc = (obj.state?.name_color as string) || "";
    if (nc && rawName.length > 0) {
        return `${nc}${rawName[0]}%cn%ch%cw${rawName.slice(1)}%cn`;
    }
    return rawName;
}
...
export default async (u: IUrsamuSDK) => {
    ...
    if (rawinput.includes("=") && switches.length === 0) {
        ...
        const cName = coloredName(target, target.name || targetName);
        u.send(`%chMAIL:%cn Draft started to ${cName}. Subject: ${subject || "(No Subject)"}`);
    ...
    switch(subCmd) {
        case "list": {
            ...
            if (mails.length === 0) {
                ...
                for (const m of mails) {
                    ...
                    const fromName = coloredName(fromObj, fromObj?.name || "Unknown");
                    const rawFromName = fromObj?.name || "Unknown";
                    ...
                    // Format line — pad based on raw name length for alignment
                    const num = String(i).padStart(3);
                    const namePad = " ".repeat(Math.max(1, 16 - rawFromName.length));
                    const sub = `Sub:  ${(m.subject || "(No Subject)").slice(0, 28)}`;

                    u.send(`${flags} ${num} ${charStr}   From: ${fromName}${namePad} ${sub}`);
                    i++;
                }
            }
        ...
        case "forward": {
            ...
            const cName = coloredName(target, target.name || targetName);
            u.send(`%chMAIL:%cn Message forwarded to ${cName}.`);
        ...
        case "cc": {
            ...
            await u.db.modify(en.id, "$set", { "data.tempMail": draft });
            const cName = coloredName(target, target.name || targetName);
            u.send(`%chMAIL:%cn Added ${cName} to CC.`);
            break;
        }
        ...
        case "bcc": {
            ...
            await u.db.modify(en.id, "$set", { "data.tempMail": draft });
            const cName = coloredName(target, target.name || targetName);
            u.send(`%chMAIL:%cn Added ${cName} to BCC.`);
            break;
        }
        ...
        case "draft": {
            ...
            await u.db.modify(en.id, "$set", { "data.tempMail": draft });
            const cName = coloredName(target, target.name || targetName);
            u.send(`%chMAIL:%cn Draft started to ${cName}.`);
        ...
        case "send": {
            ...
            const senderName = coloredName(en, en.name || "Unknown");
            const allTo = [...draft.to, ...(draft.cc || []), ...(draft.bcc || [])];
            for (const tId of allTo) {
                const id = tId.replace("#", "");
                if (id !== en.id) {
                    u.send(`%chMAIL:%cn You have a new message from ${senderName}`, id);
                }
            }
        ...
        case "proof": {
            ...
            const resolveNames = async (ids: string[]): Promise<string> => {
                ...
                for (const id of ids) {
                    ...
                    names.push(coloredName(obj, obj?.name || id));
                }
                return names.join(", ");
            };

            u.send(`\n%chDRAFT PREVIEW%cn`);
            u.send(`From:    ${coloredName(en, en.name || "Unknown")} (#${en.id})`);
